...
RX_NOISE_VOLTAGE_FROM_EC_COM=[]; RX_NOISE_FROM_EC_dB_COM=[]; SNR_AFTER_EC_COM=[];
ECHO_CANCELATION_COM=[]; MSE_COM=[]; E_COM=[];
%%%%%%%%%%%%% SET UP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%% SET UP TRAINING LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tl_n=0;
tl_n=1%for tl_n=1:1:tl_itterations
    clearvars -except ALG ...
        ...
        sout_NP_GDO_EC_COM s_at_receiver_Post_AGC_COM s_at_receiver_PCAEC_COM RX_NOISE_VOLTAGE_FROM_EC_COM RX_NOISE_FROM_EC_dB_COM SNR_AFTER_EC_COM ECHO_CANCELATION_COM MSE_COM E_COM
%%%%%%%%%%%%% SET UP TRAINING LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    sl_n=0; sl_n_end=[]; START_OFFSET=[]; FILTER_OFFSET=[];
    seqlen=((NSYMBOLS+(2*length(PS)))*USAMPR)+(NSYMBOLS_LONG_FILTER*USAMPR);
    if length(NTAPS_OR_PREVEQ)==1
        FILTER_OFFSET=((NTAPS_OR_PREVEQ-1)/2);
        sl_n_end=length(s_at_receiver_Post_AGC)-FILTER_OFFSET;
    ...
    for sl_n=START_OFFSET:1:sl_n_end
        ...
        switch ALG
            case 'lms'
            %LMS
                [channel_model MSE]=LMS_Channel_Model(s_at_receiver_Post_AGC(vector_values),sout_NP_GDO(vector_values),NTAPS_OR_PREVEQ,mu,1);
                NTAPS_OR_PREVEQ=channel_model;
            case 'nlms'
            %Normalized LMS
                [channel_model MSE CURRENT]=NLMS_Channel_Model(s_at_receiver_Post_AGC(vector_values),sout_NP_GDO(vector_values),NTAPS_OR_PREVEQ,mu,1);
                NTAPS_OR_PREVEQ=channel_model;
            case 'ls'
            %LS
                [channel_model MSE RES]=LS_Channel_Model(s_at_receiver_Post_AGC(vector_values),sout_NP_GDO(vector_values),NTAPS_OR_PREVEQ);
            case 'rls'
            %RLS
                %[channel_model MSE Previous_P]=RLS_Channel_Model(s_at_receiver_Post_AGC(vector_values),sout_NP_GDO(vector_values),NTAPS_OR_PREVEQ,mu,epsilon,Previous_P,1);
                %NTAPS_OR_PREVEQ=channel_model; epsilon=[];
                
                e=[]; CURRENT=[]; DESIRED=[]; current_vec=[]; gain=[]; P=[];
                mu_inv=1/mu;

                if sum(size(NTAPS_OR_PREVEQ))==length(size(NTAPS_OR_PREVEQ))
                    channel_model=ones(1,NTAPS_OR_PREVEQ);
                    P=eye(NTAPS_OR_PREVEQ)*epsilon; Previous_P=[];
                else
                    channel_model=NTAPS_OR_PREVEQ;
                    P=Previous_P; epsilon=[];
                end

                current_vec=sout_NP_GDO(vector_values);
                CURRENT=channel_model*current_vec.';
                DESIRED=s_at_receiver_Post_AGC(sl_n);
                e=DESIRED-CURRENT;
                gain=(mu_inv*P*current_vec.')/(1+mu_inv*(current_vec'.'*(P*current_vec.')));
                channel_model=channel_model+gain'*e;
                Previous_P=mu_inv*(P-gain*(current_vec'.'*P));
        ...
        sout_NP_GDO_EC=CURRENT;%FOR RLS TESTING ONLY %channel_model*sout_NP_GDO(vector_values).';
        ...
        s_at_receiver_PCAEC=s_at_receiver_Post_AGC(sl_n)-sout_NP_GDO_EC;
        E_COM=[E_COM s_at_receiver_PCAEC];
        MSE_COM=[MSE_COM 10*log10((E_COM*E_COM')/length(E_COM))];
        
        RX_NOISE_VOLTAGE_FROM_EC=s_at_receiver_PCAEC-(sin_NP_GDO_PAWGN(sl_n).*COMPOSITE_RX_AGC_V_GAIN);
        RX_NOISE_FROM_EC_dB=10*log10(((s_at_receiver_PCAEC)*(s_at_receiver_PCAEC)')/length(s_at_receiver_PCAEC))...
            -10*log10(((RX_NOISE_VOLTAGE_FROM_EC)*(RX_NOISE_VOLTAGE_FROM_EC)')/length(RX_NOISE_VOLTAGE_FROM_EC));
        SNR_AFTER_EC=One_Ohm_System_Power_dBm(s_at_receiver_PCAEC)-One_Ohm_System_Power_dBm(RX_NOISE_VOLTAGE_FROM_EC);
        ...
        s_at_receiver_Post_AGC_COM=[s_at_receiver_Post_AGC_COM s_at_receiver_Post_AGC(sl_n)];
        s_at_receiver_PCAEC_COM=[s_at_receiver_PCAEC_COM s_at_receiver_PCAEC];
        RX_NOISE_VOLTAGE_FROM_EC_COM=[RX_NOISE_VOLTAGE_FROM_EC_COM RX_NOISE_VOLTAGE_FROM_EC];
...
active_vector=FILTER_OFFSET:1:sl_n_end;
TX_SIGNAL_SPECTRUM_TRANSMITTED=lconv(averaging_filter,20*log10(fftshift(abs(fft(sout_NP_GDO(active_vector),length(sout_NP_GDO(active_vector)))))),'full')-10*log10(0.001);
TX_SIGNAL_POWER_TRANSMITTED=sum(TX_SIGNAL_SPECTRUM_TRANSMITTED)/length(TX_SIGNAL_SPECTRUM_TRANSMITTED);
TX_SIGNAL_SPECTRUM_SELF_RECEIVED=lconv(averaging_filter,20*log10(fftshift(abs(fft(sout_NP_GDO_PC(active_vector),length(sout_NP_GDO_PC(active_vector)))))),'full')-10*log10(0.001);
TX_SIGNAL_POWER_SELF_RECEIVED=sum(TX_SIGNAL_SPECTRUM_SELF_RECEIVED)/length(TX_SIGNAL_SPECTRUM_SELF_RECEIVED);
TX_SIGNAL_SPECTRUM_DATA_RECEIVED=lconv(averaging_filter,20*log10(fftshift(abs(fft(sin_NP_GDO_PAWGN(active_vector),length(sin_NP_GDO_PAWGN(active_vector)))))),'full')-10*log10(0.001);
...
RX_SIGNAL_NOISE_SPECTRUM_COMPOSITE_POST_AGC_AND_EC=lconv(averaging_filter,20*log10(fftshift(abs(fft(RX_NOISE_VOLTAGE_FROM_EC_COM,length(RX_NOISE_VOLTAGE_FROM_EC_COM))))),'full')-10*log10(0.001);
...
figure(2), plot(abs(sout_NP_GDO(active_vector))), hold on, plot(abs(sout_NP_GDO_PC(active_vector)),'r'), plot(abs(sin_NP_GDO_PAWGN(active_vector)),'y'), plot(abs(s_at_receiver_Post_AGC_COM),'g'), plot(abs(s_at_receiver_PCAEC_COM),'k'), plot(abs(RX_NOISE_VOLTAGE_FROM_EC_COM),'m'), grid on
axis_2=gca;
...
%%%%%DELETE%%%%%
hold on, plot(abs(sin_NP_GDO_PAWGN(active_vector).*COMPOSITE_RX_AGC_V_GAIN),'b')
%%%%%DELETE%%%%%
%%%%%%%%%%%%% CANCEL THE ECHO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

s_at_receiver_PCAEC_COM=[zeros(1,active_vector(1)-1) s_at_receiver_PCAEC_COM zeros(1,length(sout_NP_GDO)-active_vector(end))];
